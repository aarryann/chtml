...
function render(html, context) {
    if (!context) {
        ...
        html = html.replace(/<!--\{#context\s+src=(?<beginquote>['"])(?<src>.+?)(?<endquote>\1)\s+\/\}-->/g, (_match, _beginquote, src, _endquote) => {
...
function getContext(html) {
    html = html.replace(/<!--\{#context\s+src=(?<beginquote>['"])(?<src>.+?)(?<endquote>\1)\s+\/\}-->/g, (_match, _beginquote, _src, _endquote) => {
...
function parse(template, context) {
    ...
    html = html.replace(/databind (.+?)\}-->([\s\S]*?)<!--\{\/databind\}-->/g, (_match, callee, fragment) => {
        // Invoke the function, pass the enclosed HTML as argument and render the returned value
        return context[callee](fragment);
    });
    // Replace each #each block with its rendered contents
    html = html.replace(/each (.+?)\}-->([\s\S]*?)<!--\{\/each\}-->/g, (_match, arrayKey, itemTemplate) => {
    ...
    html = html.replace(/if (.+?)\}-->([\s\S]*?)<!--\{\/if\}-->/g, (_match, condition, content) => {
